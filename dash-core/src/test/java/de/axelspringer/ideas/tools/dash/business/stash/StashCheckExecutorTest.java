...
public class StashCheckExecutorTest {
    ...
    @Test
    public void execute_check_with_two_repos_and_three_merge_requests_results_in_one_check_with_two_teams() throws Exception {
    	givenOneReposAndOneMergeRequest();

        whenCheckIsExecutedWithTwoTeams();

        thenResultHasOneCheckResults();
    }
    ...
    private void givenOneReposAndOneMergeRequest() {
        ...
        doReturn(stashRepos).when(stashApiClient).readStashRepos(any(StashConfig.class));
        doReturn(asList(new StashPullRequest("1", oneRepo))).when(stashApiClient).readStashPullRequests(GIVEN_STASH_CONFIG, oneRepo);
    }
    
    private void whenCheckIsExecuted() {
        checkResult = stashCheckExecutor.executeCheck(new StashCheck("myCheck", FailureGroup.INSTANCE, Arrays.asList(TestTeam.INSTANCE), GIVEN_STASH_CONFIG));
    }
    
    private void whenCheckIsExecutedWithTwoTeams() {
        checkResult = stashCheckExecutor.executeCheck(new StashCheck("myCheck", FailureGroup.INSTANCE, Arrays.asList(TestTeam.INSTANCE, TestTeam.INSTANCE), GIVEN_STASH_CONFIG));
    }
    ...
    private void thenResultHasOneCheckResults() {
        ...
        assertEquals(State.RED, checkResult1.getState());
        assertEquals(TestTeam.INSTANCE, checkResult1.getTeams().get(0));
        assertEquals(TestTeam.INSTANCE, checkResult1.getTeams().get(1));
    }
}